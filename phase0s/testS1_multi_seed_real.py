...
def main():
    ...
    print("=" * 70)
    print("S1 — Multi-Seed Variance at Real Scale")
    print("=" * 70)
    ...
    for seed in seeds:
        ...
        for batch_start in range(0, N_SAMPLES, BATCH_SIZE):
            ...
            z_final = sde_sample_with_trajectory(
                ...
                noise=noise, return_trajectory=False)
        ...
        seed_degs.append(deg_rate)
        seed_ppls.append(mean_ppl)
        
        print(f"  Degeneracy: {deg_rate:.1%}")
        print(f"  Valid PPL:  {mean_ppl:.2f}")

    # Compute variance
    deg_mean = np.mean(seed_degs)
    deg_std = np.std(seed_degs, ddof=1)
    
    ppl_mean = np.mean(seed_ppls)
    ppl_std = np.std(seed_ppls, ddof=1)
    
    print("\n" + "=" * 70)
    print(f"S1 RESULTS (N={N_SAMPLES} per seed, 6 seeds)")
    print(f"  Degeneracy: {deg_mean:.1%} ± {deg_std:.1%}")
    print(f"  Gen. PPL:   {ppl_mean:.2f} ± {ppl_std:.2f}")
    print("=" * 70)
    
    results = {
        "seeds": seeds,
        "degeneracy_rates": seed_degs,
        "ppls": seed_ppls,
        "summary": {
            "deg_mean": deg_mean,
            "deg_std": deg_std,
    ...
    with open(RESULTS_DIR / "testS1_multi_seed_real.json", "w") as f:
