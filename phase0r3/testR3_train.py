...
def train_step(model, x0, input_ids, attention_mask, cond_seq_mask, config, device, label_smoothing=0.0, entropy_weight=0.0):
    ...
    if entropy_weight > 0.0:
        probs = F.softmax(decoder_logits_float, dim=-1)
        log_probs = F.log_softmax(decoder_logits_float, dim=-1)
        entropy = -(probs * log_probs).sum(dim=-1)
        # add penalty for low entropy (i.e. minimize -entropy)
        ce_per_token = ce_per_token - entropy_weight * entropy
...
def finetune_toy_model(embeddings, token_ids, vocab_size, label_smoothing, entropy_weight, ckpt_name):
    print("=" * 60)
    print(f"R3 — Training {ckpt_name} (LS={label_smoothing}, EW={entropy_weight})")
    print("=" * 60)
    ...
    for step in range(NUM_TRAIN_STEPS):
        ...
        loss, l2_loss, ce_loss = train_step(
            model, x0, ids, attention_mask, cond_seq_mask, config, DEVICE, label_smoothing, entropy_weight
        )
        loss.backward()
...
def main():
    ...
    finetune_toy_model(embeddings, token_ids, vocab_size, 0.0, 0.0, "testR3_model_control.pt")
    finetune_toy_model(embeddings, token_ids, vocab_size, 0.1, 0.0, "testR3_model_label_smooth.pt")
    finetune_toy_model(embeddings, token_ids, vocab_size, 0.0, 0.1, "testR3_model_entropy_reg.pt")
