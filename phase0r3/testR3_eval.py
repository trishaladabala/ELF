...
from phase0r.testR1_eval import generate_and_track, detect_degeneracy, proportion_confint, compute_ppl_distilgpt2, bootstrap_ci, bootstrap_ci_paired_diff, load_toy_model, DummyConfig
...
def evaluate_model(ckpt_name, label, n_samples=768):
    ...
    # Custom load to change RESULTS_DIR
...
def main():
    res_ctrl = evaluate_model("testR3_model_control.pt", "Control Model", n_samples=768)
    res_ls = evaluate_model("testR3_model_label_smooth.pt", "Label Smoothing (0.1)", n_samples=768)
    res_ent = evaluate_model("testR3_model_entropy_reg.pt", "Entropy Reg (0.1)", n_samples=768)
    
    out_path = RESULTS_DIR / "testR3_confidence_calibration.json"
    save_dict = {
        "control": {k: v for k, v in res_ctrl.items() if k not in ["ode_log_ppls", "sde_log_ppls"]},
        "label_smooth": {k: v for k, v in res_ls.items() if k not in ["ode_log_ppls", "sde_log_ppls"]},
        "entropy_reg": {k: v for k, v in res_ent.items() if k not in ["ode_log_ppls", "sde_log_ppls"]}
    }
    with open(out_path, "w") as f:
        json.dump(save_dict, f, indent=2)
        
    print(f"\nSaved R3 evaluation to {out_path}")
    
    diffs_ctrl = res_ctrl["sde_log_ppls"] - res_ctrl["ode_log_ppls"]
    diffs_ls = res_ls["sde_log_ppls"] - res_ls["ode_log_ppls"]
    diffs_ent = res_ent["sde_log_ppls"] - res_ent["ode_log_ppls"]
    
    mean_diff_ls, lo_ls, hi_ls = bootstrap_ci_paired_diff(diffs_ls, diffs_ctrl)
    mean_diff_ent, lo_ent, hi_ent = bootstrap_ci_paired_diff(diffs_ent, diffs_ctrl)
    
    print("\n--- Go/No-Go Criterion Check ---")
    print(f"Control SDE-ODE Gap: {res_ctrl['sde_penalty']:.4f}")
    
    print(f"\nLabel Smoothing Gap: {res_ls['sde_penalty']:.4f}")
    print(f"Difference (LS - Control): {mean_diff_ls:.4f} CI: ({lo_ls:.4f}, {hi_ls:.4f})")
    
    print(f"\nEntropy Reg Gap: {res_ent['sde_penalty']:.4f}")
    print(f"Difference (Ent - Control): {mean_diff_ent:.4f} CI: ({lo_ent:.4f}, {hi_ent:.4f})")
