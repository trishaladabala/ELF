...
def evaluate_model(ckpt_name, label):
    print(f"\n{'='*40}")
...
def main():
    res_std = evaluate_model("testR2_model_control.pt", "Control Model")
    res_over = evaluate_model("testR2_model_reg.pt", "Curvature-Regularized (0.1)")
    res_05 = evaluate_model("testR2_model_reg_0.05.pt", "Curvature-Regularized (0.05)")
    res_50 = evaluate_model("testR2_model_reg_0.5.pt", "Curvature-Regularized (0.5)")
    
    # Save results
    out_path = RESULTS_DIR / "testR2_sensitivity_check.json"
    ...
    print(f"\nSaved R2 sensitivity evaluation to {out_path}")
    
    print("\n--- Sensitivity Check ---")
    print(f"Control SDE Penalty: {res_std['sde_penalty']:.4f} (Curvature: {res_std['avg_ode_curvature']:.4f})")
    print(f"Reg 0.05 SDE Penalty: {res_05['sde_penalty']:.4f} (Curvature: {res_05['avg_ode_curvature']:.4f})")
    print(f"Reg 0.10 SDE Penalty: {res_over['sde_penalty']:.4f} (Curvature: {res_over['avg_ode_curvature']:.4f})")
    print(f"Reg 0.50 SDE Penalty: {res_50['sde_penalty']:.4f} (Curvature: {res_50['avg_ode_curvature']:.4f})")
