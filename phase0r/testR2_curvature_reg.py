...
Fine-tunes the C4 toy model with an auxiliary curvature penalty.
Compares a control model (curvature_weight=0) against a regularized model.
"""
...
# Training
NUM_TRAIN_STEPS = 5000  # Fine-tuning steps
BATCH_SIZE = 32
LR = 1e-3
DECODER_PROB = 0.5
T_EPS = 0.05
...
def train_step(model, x0, input_ids, attention_mask, cond_seq_mask, config, device, curvature_weight=0.0):
    ...
    if curvature_weight > 0:
        ...
        # We step z backward along the denoising path: z_prev = z - dt * v
    ...
    loss = total_sum / torch.clamp(loss_mask_f.sum(), min=1.0) + curvature_weight * curvature_loss
    ...
    return loss, l2_loss, ce_loss, curvature_loss


def finetune_toy_model(embeddings, token_ids, vocab_size, curvature_weight, ckpt_name):
    print("=" * 60)
    print(f"R2 — Fine-tuning with curvature_weight={curvature_weight}")
    print("=" * 60)
    ...
    rng = np.random.default_rng(42)
    l2_losses, ce_losses, total_losses, curv_losses = [], [], [], []
    LOG_FREQ = 500
    t0 = time.time()
    ...
    for step in range(NUM_TRAIN_STEPS):
        ...
        optimizer.zero_grad()
        loss, l2_loss, ce_loss, curv_loss = train_step(
        ...
        curv_losses.append(float(curv_loss.item()))
        ...
        if (step + 1) % LOG_FREQ == 0:
            ...
            print(f"  Step {step+1:5d}/{NUM_TRAIN_STEPS}: "
                  ...
                  f"ce={avg_ce:.4f}, "
                  f"curv={np.mean(curv_losses[-LOG_FREQ:]):.4f}, "
    ...
    plt.suptitle(f"R2: Curvature Weight {curvature_weight} ({NUM_TRAIN_STEPS} steps)",
                 fontsize=13, y=1.02)
    plt.tight_layout()
    plt.savefig(RESULTS_DIR / f"{ckpt_name}.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  Saved plot → {RESULTS_DIR / f'{ckpt_name}.png'}")
    ...
    with open(RESULTS_DIR / f"{ckpt_name}.json", "w") as f:
...
def main():
    ...
    # Fine-tune a control (0.0) and a regularized model (0.1) - these might already exist but we can re-run or just run the new ones.
    # To save time, we will only run the new ones if the old ones exist, but the prompt says "rerun the curvature-regularized fine-tune at two additional penalty weights".
    # I will just run all of them or only the new ones? Wait, it's safer to just let it train the new ones. But I'll leave the original ones in case someone runs this script from scratch.
    finetune_toy_model(embeddings, token_ids, vocab_size, 0.0, "testR2_model_control.pt")
    finetune_toy_model(embeddings, token_ids, vocab_size, 0.1, "testR2_model_reg.pt")
    # Sensitivity check
    finetune_toy_model(embeddings, token_ids, vocab_size, 0.05, "testR2_model_reg_0.05.pt")
    finetune_toy_model(embeddings, token_ids, vocab_size, 0.5, "testR2_model_reg_0.5.pt")
    return 0
