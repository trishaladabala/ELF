...
Trains two toy checkpoints:
(a) ELF's default logit-normal schedule
(b) logit_normal_oversampled schedule (extra density near t=1)
"""
...
def train_toy_model(embeddings, token_ids, vocab_size, schedule_name, ckpt_name):
    print("=" * 60)
    print(f"R1 — Training with schedule: {schedule_name}")
    print("=" * 60)
    ...
    config = ToyConfig()
    config.time_schedule = schedule_name
    ...
    plt.suptitle(f"R1: {schedule_name} ({NUM_TRAIN_STEPS} steps)",
                 fontsize=13, y=1.02)
    plt.tight_layout()
...
def main():
    ...
    train_toy_model(embeddings, token_ids, vocab_size, "logit_normal", "testR1_model_std.pt")
    train_toy_model(embeddings, token_ids, vocab_size, "logit_normal_oversampled", "testR1_model_over.pt")
    return 0
